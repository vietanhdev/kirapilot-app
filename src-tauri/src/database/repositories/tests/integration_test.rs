...
mod integration_tests {
    ...
    use crate::database::repositories::{
        TaskRepository, TimeTrackingRepository, AiRepository,
        task_repository::CreateTaskRequest,
        time_tracking_repository::CreateTimeSessionRequest,
        ai_repository::CreateAiInteractionRequest,
    };
    use chrono::Utc;
    ...
    async fn test_task_repository_integration() {
        ...
        let task = repo.create_task(request).await.expect("Failed to create task");
        assert_eq!(task.title, "Test Task");
        assert_eq!(task.priority, 1);

        // Find the task
        let found_task = repo.find_by_id(&task.id).await.expect("Failed to find task");
    ...
    async fn test_time_tracking_repository_integration() {
        ...
        let task = task_repo.create_task(task_request).await.expect("Failed to create task");
        ...
        let session = time_repo.create_session(session_request).await.expect("Failed to create session");
        assert_eq!(session.task_id, task.id);
        assert!(session.is_active);

        // Stop the session
        let stopped_session = time_repo.stop_session(&session.id, Some("Completed".to_string()))
            .await.expect("Failed to stop session");
        assert!(!stopped_session.is_active);
        assert!(stopped_session.end_time.is_some());

        // Clean up
        time_repo.delete_session(&session.id).await.expect("Failed to delete session");
        task_repo.delete_task(&task.id).await.expect("Failed to delete task");
    }
    ...
    async fn test_ai_repository_integration() {
        ...
        let interaction = repo.create_interaction(request).await.expect("Failed to create interaction");
        ...
        let found_interaction = repo.find_by_id(&interaction.id).await.expect("Failed to find interaction");
        ...
        // Clean up
        repo.delete_interaction(&interaction.id).await.expect("Failed to delete interaction");
    }
}