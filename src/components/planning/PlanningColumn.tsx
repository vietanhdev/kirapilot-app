  ...
  const taskIds = Children.toArray(children)
    ...
    .map(child => {
      if (child.props && typeof child.props === 'object' && 
          'task' in child.props && child.props.task && 
          typeof child.props.task === 'object' && 'id' in child.props.task) {
  ...
  const getColorClasses = () => {
    const colors = {
      slate: {
        ...
        count: 'bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-300',
        accent: 'border-slate-300 dark:border-slate-500'
      },
      blue: {
        header: 'bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-200',
        border: 'border-blue-200 dark:border-blue-700',
        count: 'bg-blue-200 dark:bg-blue-800 text-blue-800 dark:text-blue-200',
        accent: 'border-blue-400 dark:border-blue-500'
      },
      green: {
        header: 'bg-green-50 dark:bg-green-900/20 text-green-800 dark:text-green-200',
        border: 'border-green-200 dark:border-green-700',
        count: 'bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-200',
        accent: 'border-green-400 dark:border-green-500'
      },
      purple: {
        header: 'bg-purple-50 dark:bg-purple-900/20 text-purple-800 dark:text-purple-200',
        border: 'border-purple-200 dark:border-purple-700',
        count: 'bg-purple-200 dark:bg-purple-800 text-purple-800 dark:text-purple-200',
        accent: 'border-purple-400 dark:border-purple-500'
      },
      orange: {
        header: 'bg-orange-50 dark:bg-orange-900/20 text-orange-800 dark:text-orange-200',
        border: 'border-orange-200 dark:border-orange-700',
        count: 'bg-orange-200 dark:bg-orange-800 text-orange-800 dark:text-orange-200',
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center space-x-1">
                <h3 className={`font-semibold text-base truncate ${isToday ? 'text-primary-600 dark:text-primary-400' : 'text-gray-900 dark:text-gray-100'}`}>
                  {title}
                  {subtitle && (
                    <span className={`ml-1 text-sm font-normal ${isToday ? 'text-primary-500 dark:text-primary-400' : 'text-gray-500 dark:text-gray-400'}`}>
                      {subtitle}
                    </span>
                  )}
                </h3>
