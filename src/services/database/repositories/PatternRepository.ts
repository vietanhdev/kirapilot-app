...
export class PatternRepository {
  ...
  async recordProductivityData(
    ...
    return await executeTransaction(async db => {
      ...
      if (existing.length > 0) {
        ...
        const newScore =
          (existingPattern.productivity_score * existingPattern.sample_size +
            productivityScore) /
          newSampleSize;
        const newConfidence = Math.min(
          (existingPattern.confidence_level * existingPattern.sample_size +
            confidenceLevel) /
            newSampleSize,
          1.0
        );
  ...
  async getProductivityTrend(
    ...
    for (const task of taskTrend) {
      const productivity =
        task.completed_tasks * 10 + (1 / task.efficiency) * 50;
  ...
  private generateInsights(
    ...
    const mostProductivePattern = patterns.reduce(
      (max, pattern) =>
        pattern.productivity > max.productivity ? pattern : max,
      patterns[0] || {
        timeSlots: [{ start: '09:00', end: '10:00', dayOfWeek: 1 }],
        productivity: 0,
      }
    );

    // Find least productive time
    const leastProductivePattern = patterns.reduce(
      (min, pattern) =>
        pattern.productivity < min.productivity ? pattern : min,
      patterns[0] || {
        timeSlots: [{ start: '15:00', end: '16:00', dayOfWeek: 1 }],
        productivity: 100,
      }
    );
  ...
  private async generateRecommendations(
    ...
    if ((insights.focusEfficiency as number) < 70) {
      recommendations.push({
        ...
        description:
          'Your focus efficiency is below optimal. Consider shorter, more frequent focus sessions.',
        confidence: 80,
        actionable: true,
        priority: 2,
        estimatedImpact: 25,
