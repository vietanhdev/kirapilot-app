...
export class TimeTrackingRepository {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (
          ...
          console.warn(
            `Database locked, retrying attempt ${attempt + 1}/${maxRetries}`
          );
          continue;
        }
        throw error;
  ...
  async startSession(taskId: string, notes?: string): Promise<TimerSession> {
    return await executeTransaction(async db => {
      ...
      if (!validation.success) {
        ...
        throw new Error(
          `Invalid session data: ${validation.error.issues.map(i => i.message).join(', ')}`
        );
      }
  ...
  async pauseSession(sessionId: string): Promise<TimerSession> {
    ...
    if (!session.isActive) {
      console.warn(
        `Session ${sessionId} is already paused, returning current state`
      );
    ...
    if (updateResult.rowsAffected === 0) {
      ...
      throw new Error(
        'Failed to update session - session ID not found or WHERE clause failed'
      );
    }
    ...
    if (verifyResult.length > 0 && verifyResult[0].is_active) {
      throw new Error(
        'Failed to update session state - database inconsistency'
      );
    }
  ...
  async resumeSession(sessionId: string): Promise<TimerSession> {
    ...
    if (session.isActive) {
      console.warn(
        `Session ${sessionId} is already active, returning current state`
      );
